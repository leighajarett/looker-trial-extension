...
const InternalCalendar = ({ locale = 'en', onDayClick, className, size, showNextButton = true, showPreviousButton = true, onMonthChange, onNextClick, onNowClick, onPrevClick, viewMonth, selectedDates, }) => {
    ...
    return (React.createElement(CalendarContext.Provider, { value: {
        ...
        React.createElement(DayPicker, { className: `${renderDateRange && 'render-date-range'} ${className}`, selectedDays: selectedDates, localeUtils: MomentLocaleUtils, locale: locale, month: viewMonth, showOutsideDays: true, onDayClick: onDayClick, navbarElement: CalendarNav, captionElement: NoopComponent, modifiers: modifiers, onMonthChange: onMonthChange })));
};
export const Calendar = styled(InternalCalendar).withConfig({ displayName: "Calendar", componentId: "sc-13nwwp2" }) `
  ${reset}
  ${calendarSpacing}
  ...
  &.render-date-range {
    .DayPicker-Day--selected {
      ...
      &:not(.DayPicker-Day--to):not(.DayPicker-Day--from) {
        ...
        color: ${({ theme }) => mix(0.65, theme.colors.semanticColors.primary.light, theme.colors.semanticColors.neutral.altText)};
      }
